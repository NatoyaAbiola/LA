...
10.times do
  comments_data << Faker::Superhero.name
end
...
comments_data.each do |comment|
  db_connection do |conn|
    ...
    conn.exec_params(sql, [Faker::Superhero.name, name, id])
  end
end
...
puts "Recipe name with the comment(Bizarro) is '#{@specific_name[0]["recipe_name"]}'"
