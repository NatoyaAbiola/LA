...
post "/meetups" do
  ...
  if @meetups.save
    ...
    if @meetups.creator.nil?
      @error_message = "Please sign in to create a  new meetup!"
    elsif @meetups.name == ""
      @error_message = "Please enter the name of your meetup!"
    elsif @meetups.location == ""
      @error_message = "Please enter the location for your meetup!"
...
post '/meetups/:id' do
  ...
  if current_user.nil?
    flash[:notice] = "Please sign in to join this meetup"
  elsif @users.include?(current_user.id)
   flash[:notice] = "You are already a member of this meetup!"
  elsif current_user
    members =  Membership.create(user_id: current_user.id, meetup_id: params[:id])
